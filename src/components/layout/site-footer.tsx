    ...
    <footer className="mt-16 border-t-4 border-accent-600 bg-primary-900 text-white" data-print="hide">
      ...
      <div className="container-page grid gap-10 py-12 lg:grid-cols-12 lg:gap-8">
        ...
        <section className="lg:col-span-4" aria-labelledby="footer-contact">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <address className="mt-2 not-italic text-white/85">
                {site.address.street}
                <br />
          ...
          <ul className="mt-4 flex flex-col gap-2 text-white/85">
            <li className="flex items-center gap-2">
              <Icon name="phone" size={17} />
              <a href={`tel:${site.contact.phoneE164}`} className="text-white underline underline-offset-[0.2em]">
                {site.contact.phone}
              </a>
              <span className="text-sm text-white/60">({site.contact.callCost})</span>
            </li>
            <li className="flex items-center gap-2">
              <Icon name="mail" size={17} />
              <a
                href={`mailto:${site.contact.email}`}
                className="break-all text-white underline underline-offset-[0.2em]"
              >
                {site.contact.email}
              </a>
            </li>
          ...
          <a
            ...
            className="mt-4 flex items-center gap-2 rounded-md border border-white/25 px-3 py-2.5 text-white hover:border-white/60"
          >
          ...
          <dl className="mt-2 text-white/85">
            ...
            {site.emergency.map((entry) => (
              <li key={entry.number + entry.label} className="flex flex-col">
                <a
                  ...
                  className="font-semibold text-white underline underline-offset-[0.2em] tabular-nums"
                >
                  {entry.number}
                </a>
                <span className="text-sm text-white/75">{entry.label}</span>
              </li>
            ))}
          </ul>
        </section>
            ...
            {freguesias.map((freguesia) => (
              <li key={freguesia.slug}>
                <Link
                  ...
                  className="text-sm text-white/85 no-underline hover:text-white hover:underline underline-offset-[0.2em]"
                >
        ...
        <section className="lg:col-span-3" aria-labelledby="footer-newsletter">
          ...
          <p className="mt-2 mb-4 text-white/85">{dict.footer.newsletterLead}</p>
            ...
            {site.social.map((entry) => (
              <li key={entry.label}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm text-white/85 underline underline-offset-[0.2em] hover:text-white"
                >
                  {entry.label}
      ...
      <div className="border-t border-white/15">
        <div className="container-page py-4">
          <details className="group">
            <summary className="inline-flex min-h-11 cursor-pointer list-none items-center gap-2 text-sm text-white/80 hover:text-white">
              <Icon
                name="chevronRight"
                size={16}
              ...
              <span className="text-white/50">— {dict.footer.partnersToggle}</span>
            </summary>
            <ul className="mt-4 flex flex-wrap gap-3">
              {[
                ...
                <li
                  key={partner}
                  className="rounded-md border border-white/20 px-3 py-2 text-sm text-white/75"
                >
                  {partner}
                </li>
              ))}
      ...
      <div className="border-t border-white/15">
        <div className="container-page flex flex-col gap-4 py-5 md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-white/70">{fill(dict.footer.copyright, { year })}</p>
            ...
            <ul className="flex flex-wrap gap-x-5 gap-y-2">
              ...
              <li>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-sm text-white/85 underline underline-offset-[0.2em] hover:text-white"
                >
